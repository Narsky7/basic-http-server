...
http_parse_e read_http_request(int socket_fd, http_request *request) {
  ssize_t bytes_read =
      read(socket_fd, request->raw_request, sizeof(request->raw_request) - 1);

  if (bytes_read <= 0) {
    return HTTP_PARSE_INVALID;
  }

  request->raw_request[bytes_read] = '\0';

  if (sscanf(request->raw_request, "%7s %2047s %15s", request->method,
             request->path, request->protocol) != 3) {
    return HTTP_PARSE_INVALID;
  }
...
void sanitize_path(const char *requested_path, char *sanitized_path,
  ...
  if (strcmp(requested_path, "/") == 0) {
    snprintf(sanitized_path, buffer_size, "%s/index.html", web_root);
  } else {
...
void serve_file(const char *path, http_response *response) {
  ...
  if (!file) {
    ...
    if (!file) {
      perror("Failed to open 404 fallback file");
      exit(EXIT_FAILURE);
    }
  }
